...
def liberar_barramento(nome_thread):
    ...
    registrar_log(nome_thread, "Liberou o barramento e devolveu a prioridade.")
    not_busy.notify()
...
def thread_baixa():
    ...
    adquirir_barramento(THREAD_BAIXA, False)
...
def thread_media():
    ...
    while(prioridade_herdada):
        registrar_log(THREAD_MEDIA, "BLOQUEADA! Thread Baixa esta com prioridade ALTA.")
        time.sleep(0.05)

    cpu_ocupada = True

    contador = 0
    # Mesmo peso de processamento (50 milhões) para a comparação ser justa
...
def thread_alta():
    ...
    adquirir_barramento(THREAD_ALTA, True)
...
def main():
    print("--- INICIANDO SIMULADOR MARS PATHFINDER (COM SOLUCAO) ---\n")
